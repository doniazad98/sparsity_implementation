...
def len_corated_items(test,train, user_id, v):
    ...
    return len(co_rated)
def similarity_matrice_20():
    test_set = load_eval_data("test_set", 4)
    train_set = load_eval_data("train", 4)
    pairs = load_eval_data("pairs", 4)
    actual = load_eval_data("actual", 4)
    # local_similarity(train_set,5)
...
def sparsity2article(test,train,user_id):
...
def local_similarity(test,train,user_id,item_list,users):
    sim=0
    up_sim=0
    down_sim_p=0
    down_sim_q=0
    similarity=[]
    u_user=test.loc[user_id, :]
    u_sp=user_surprisal_vector(train, u_user, item_list)
    #print("the vector is {} ".format(u_sp))
    #print("the value is {} ".format(u_sp.loc[1,'valeur']))
    for v in users :
        sim=0
        up_sim = 0
        down_sim_p = 0
        down_sim_q = 0
        v_user=train.loc[v, :]
        v_sp=user_surprisal_vector(train, v_user, item_list)
        for i in item_list:
            if u_user.loc[i]!= 0.0 and v_user.loc[i] != 0.0:
                up_sim=up_sim+u_sp.loc[i,'valeur']*v_sp.loc[i,'valeur']
                down_sim_p=down_sim_p+u_sp.loc[i,'valeur']*u_sp.loc[i,'valeur']
                down_sim_q=down_sim_q+v_sp.loc[i,'valeur']*v_sp.loc[i,'valeur']

                #print("down_sim_p ={} down_sim_q ={} up_sim={} ".format(down_sim_p ,down_sim_q,up_sim))
        if down_sim_p != 0 and down_sim_q != 0:
            sim= up_sim/(sqrt(down_sim_q)*sqrt(down_sim_p))
        print("similarity of user {} is {}".format(v,sim))
        similarity.append([v,sim])
    local_similarity_2(test,train,5,similarity,user_id)
    return similarity
def local_similarity_2(test,train,corated_thresshold,similarity,user_id):
    similarity2=[]
    for i in similarity:
        co=len_corated_items(test,train,user_id,i[0])
...
def user_surprisal_vector(train,u_user,item_list):
    id_item=[]
    valeur=[]
    for item in item_list:
        rpi=u_user.loc[item]
        item_vector = train.loc[:, item]
        u,b=items_variables(item_vector.values)
        i=i_rpi(rpi,u,b)
        s=sign(rpi-u)
        id_item.append(item)
        valeur.append(s*i)
